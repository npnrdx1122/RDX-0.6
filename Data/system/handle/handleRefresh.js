    ...
    for (const file of files) {
      try {
        ...
        if (command.config && command.config.name) {
          ...
          logs.success('RDX-COMMAND', `Loaded: ${command.config.name}`);
        }
      } catch (error) {
        logs.error('RDX-COMMAND', `Failed to load ${file}: ${error.message}`);
      }
    }
    ...
    for (const file of files) {
      try {
        ...
        if (event.config && event.config.name) {
          ...
          logs.success('RDX-EVENT', `Loaded: ${event.config.name}`);
        }
      } catch (error) {
        logs.error('RDX-EVENT', `Failed to load ${file}: ${error.message}`);
      }
    }
